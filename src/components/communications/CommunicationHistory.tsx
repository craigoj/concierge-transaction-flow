...
export const CommunicationHistory = ({ query, title }: CommunicationHistoryProps) => {
  const [typeFilter, setTypeFilter] = useState<string>("all");

  const { data: communications, isLoading } = useQuery({
    queryKey: ['communication-history', query, typeFilter],
    queryFn: async () => {
      let q = query;

      if (typeFilter !== 'all') {
        q = q.eq('communication_type', typeFilter);
      }
